...
/**
 * 完成した写真をじっくり見る面（{@code MODJAM_DECISIONS_OGP.md} §32-5・KURONAMI333 承認済みの B 案）。
 *
 * <h2>歩けることをどう担保しているか</h2>
 *
 * KURONAMI333 の要件は「インベントリとかと同じで、移動しながらでも見れる」。vanilla のインベントリを
 ...
 * </ul>
 *
 * <p>幕は<b>半透明</b>で、裏の景色が見える（KURONAMI333「インベントリと全く一緒」）。値は vanilla の
...
public final class PhotographViewer {
     ...
     * （2026-08-23 KURONAMI333 実機指摘）。同じ右クリックで実際に起こりうる操作なので、
     * 音が似ているだけで誤解が成立してしまう。
     *
