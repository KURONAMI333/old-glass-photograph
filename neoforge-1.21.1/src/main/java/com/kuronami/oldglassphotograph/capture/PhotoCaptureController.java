...
public final class PhotoCaptureController {
    ...
    public static void openShutter(ServerPlayer player, ShutterRequestPayload payload) {
        ...
        // 26.x の isWithinBlockInteractionRange(pos, margin) は 1.21.1 に無いので
        // blockInteractionRange() で同じ意味の距離判定をここで行う（マージン 2.0 も同じ）。
        Vec3 center = Vec3.atCenterOf(basePos);
        double maxRange = player.blockInteractionRange() + 2.0;
        if (player.distanceToSqr(center.x, center.y, center.z) > maxRange * maxRange) {
    ...
    public static void receivePixels(ServerPlayer player, PhotoMapPixelsPayload payload) {
        ...
        if (session == null) {
            LOG.warn("[ogp] unknown or already-used capture token {} from {}",
                    payload.token(), player.getGameProfile().getName());
            return;
        }
