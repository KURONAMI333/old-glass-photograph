...
public final class PhotoCaptureClient {
     ...
     * 何も出さないと<b>フリーズしたと読まれる</b>（2026-08-22 実機指摘）。
     ...
     */
    private static final ResourceLocation VIEWFINDER_TEXTURE = ResourceLocation.fromNamespaceAndPath(
            OldGlassPhotograph.MODID, "textures/gui/viewfinder.png");

    /** viewfinder.png の実寸。blit にテクスチャ寸法を渡すのに要る（1.21.1 の blit は int UV 版のみ）。 */
    ...
    public static void openViewfinder(ViewfinderOpenPayload payload) {
        ...
        // 26.x の snapTo(yaw, pitch 同時設定) の代わりに 1.21.1 の個別 setter で置く。
     ...
     */
    public static void renderViewfinder(GuiGraphics graphics, DeltaTracker deltaTracker) {
        if (phase == Phase.IDLE) {
            return;
        }
        Minecraft mc = Minecraft.getInstance();
        ...
        //    26.x の float UV blit の代わりに 1.21.1 の int UV blit（テクスチャ寸法指定型）で
        //    同じ「矩形位置 x テクスチャ全域」を表現する。
        ...
        // 1.21.1 の blit は blend を張らないので、すりガラスの半透明が効かず面が塗り潰される
        // （26.x の GUI 描画は pipeline 側で blend が既定）。ここだけ自分で張る。
     ...
     *
      * <p>寸法は screenshot が読むのと同じ {@code getMainRenderTarget} から採る。window から採ると
    ...
    private static void finishExposure(boolean filled) {
        ...
        // 「額縁に写真をはめた」と読まれる（2026-08-23 実機指摘）。
    ...
    public static void onLevelRenderEnd() {
        ...
        // 1.21.1 の takeScreenshot は戻り値で NativeImage を返す同期版
        // （26.x の Consumer コールバック版は無い。RESOLUTION #1 の確定値）。
        // コピーは呼び出した時点で完了しているので、AfterLevel の絵がそのまま得られる。
