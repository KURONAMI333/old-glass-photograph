...
/**
 * 誰が撮って、いつ撮ったか。<b>この情報を持たない写真も普通に存在する</b>
 ...
 * KURONAMI333 の判定でゲーム内時刻でなく実世界の日時を足すことになった。
 *
 * <h2>タイムゾーンの扱い</h2>
 *
...
public record PhotoCredit(String author, long day, String capturedAt) {
    ...
    public CompoundTag save(CompoundTag tag) {
        ...
        tag.putLong("day", day);
        tag.putString("captured_at", capturedAt);
        return tag;
    }

    /** NBT から読む。{@code OgpNbt} からだけ呼ばれる。 */
    public static PhotoCredit load(CompoundTag tag) {
        return new PhotoCredit(
                tag.getString("author"),
                tag.contains("day", Tag.TAG_ANY_NUMERIC) ? tag.getLong("day") : 1L,
                tag.contains("captured_at", 8) ? tag.getString("captured_at") : "");
    }
