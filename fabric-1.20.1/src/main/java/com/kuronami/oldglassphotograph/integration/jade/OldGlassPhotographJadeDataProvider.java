...
final class OldGlassPhotographJadeDataProvider implements IServerDataProvider<BlockAccessor> {
    ...
    private static final ResourceLocation UID =
            new ResourceLocation("old_glass_photograph", "jade");
     ...
     * 板の表示名を書き込む。
     *
     * <p>板の名前に出る残り秒は表示用の写しで、server の inventoryTick が 1 秒ごとに書き直している。
     * カメラや暗室台の中の板にはその tick が回らないので、写しは箱へ入れた時刻のまま止まる。
     * 乾燥の期限は絶対時刻なので、ここでは現在の game time から引き直した値で名前を作る。
     */
    private static void writePlate(CompoundTag data, ItemStack plate, Level level) {
        ...
        if (process != null) {
            ...
            if (seconds != process.secondsLeft()) {
                shown = plate.copy();
                OgpNbt.setProcess(shown, process.withSecondsLeft(seconds));
            }
        }
